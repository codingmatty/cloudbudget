...
function updateMethod(api, model, shouldAuthenticate, callback) {
  ...
  api.put('/:id', function (req, res) {
    ...
    Model.findOneAndUpdate(query, _lodash2.default.omit(req.body, Model.readonlyProps() || []), { new: true }, function (err, doc) {
      if (err) {
        return (0, _index.handleError)(err, res, 'update', Model.modelName);
      }
      callback(req, res, doc);
    });
  });
...
function deleteMethod(api, model, shouldAuthenticate, callback) {
  ...
  api.delete('/:id', function (req, res) {
    ...
    Model.findOne(query, function (queryErr, queryDoc) {
      if (queryErr || !queryDoc) {
        return (0, _index.handleError)(queryErr, res, 'delete', model);
      }
      queryDoc.remove(req.query, function (removeErr, doc) {
        if (removeErr) {
